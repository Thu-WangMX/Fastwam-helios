...
def _load_model_config(path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    # 支持 task yaml（model config 在 model: 下）和独立 model yaml
    if "model" in cfg and "video_dit_config" in cfg.model:
        cfg = cfg.model
    if "video_dit_config" not in cfg or "action_dit_config" not in cfg:
        raise ValueError(
            f"`{path}` must contain `video_dit_config` and `action_dit_config`."
        )
...
def main() -> None:
    ...
    # Save to a local temp file first, then copy to the target path.
    # This works around NAS filesystems that do not support torch's zipfile writer.
