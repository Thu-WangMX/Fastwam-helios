...
# Probe SDPA backends once at import time. On AMD ROCm with PyTorch >= 2.3, the
# memory-efficient backend supports custom additive masks and is 2-5x faster than
# the math fallback in backward. We force the dispatcher to prefer EFFICIENT over
# MATH whenever a custom mask is present.
try:
    from torch.nn.attention import SDPBackend, sdpa_kernel
    _SDPA_PREFERRED_BACKENDS = [
        SDPBackend.FLASH_ATTENTION,
        SDPBackend.EFFICIENT_ATTENTION,
        SDPBackend.MATH,
    ]
    _SDPA_KERNEL_CTX_AVAILABLE = True
except ImportError:
    _SDPA_PREFERRED_BACKENDS = None
    _SDPA_KERNEL_CTX_AVAILABLE = False
    logger.warning(
        "torch.nn.attention.sdpa_kernel unavailable; falling back to default SDPA backend selection."
    )

# Once-only probe: profile the very first SDPA call on rank0 to log which kernel
# (Flash / mem-efficient / math) was actually dispatched. Set FASTWAM_SDPA_PROBE=0
# to disable.
import os as _os
_SDPA_PROBE_DONE = False
_SDPA_PROBE_ENABLED = _os.environ.get("FASTWAM_SDPA_PROBE", "1") != "0"
...
def _is_rank0() -> bool:
    try:
        ...
        pass
    # Fallback: env var set by torchrun / accelerate / deepspeed launchers.
    return _os.environ.get("RANK", "0") == "0" and _os.environ.get("LOCAL_RANK", "0") == "0"


def _probe_sdpa_backend(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, attn_mask):
    """Run one profiled SDPA call and log the actual GPU kernel name. rank0 only."""
    global _SDPA_PROBE_DONE
    if _SDPA_PROBE_DONE or not _SDPA_PROBE_ENABLED or not _is_rank0():
        return
    ...
    try:
        ...
        with profile(activities=activities, record_shapes=False) as prof:
            if _SDPA_KERNEL_CTX_AVAILABLE:
                with sdpa_kernel(_SDPA_PREFERRED_BACKENDS):
                    _ = F.scaled_dot_product_attention(q, k, v, attn_mask=attn_mask)
            else:
                _ = F.scaled_dot_product_attention(q, k, v, attn_mask=attn_mask)
            if q.is_cuda:
                torch.cuda.synchronize()
        # Pick out the most informative kernel name from the recorded events.
        kernel_hits = []
        for evt in prof.key_averages():
            name = evt.key.lower()
            if any(tag in name for tag in [
                "flash", "mem_eff", "memory_efficient", "efficient_attention",
                "_scaled_dot_product", "sdp", "attention",
            ]):
                kernel_hits.append((evt.key, evt.cpu_time_total, evt.device_time_total))
        # Sort by device time to surface the dominant kernel first.
        kernel_hits.sort(key=lambda x: x[2], reverse=True)
        mask_info = "None" if attn_mask is None else f"shape={tuple(attn_mask.shape)}, dtype={attn_mask.dtype}"
        logger.info(
            f"[SDPA-PROBE] q.shape={tuple(q.shape)} q.dtype={q.dtype} mask={mask_info}"
        )
        if kernel_hits:
            top_lines = "\n  ".join(
                f"{name}  cpu={cpu_us/1e3:.2f}ms  device={dev_us/1e3:.2f}ms"
                for name, cpu_us, dev_us in kernel_hits[:8]
            )
            logger.info(f"[SDPA-PROBE] dispatched kernels (top by device time):\n  {top_lines}")
        else:
            logger.warning("[SDPA-PROBE] no attention-related kernel found in profile output.")
    except Exception as e:
        logger.warning(f"[SDPA-PROBE] failed: {e!r}")
...
def flash_attention(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, num_heads: int, ctx_mask: Optional[torch.Tensor] = None, compatibility_mode=True):
    """SDPA wrapper that prefers Flash / memory-efficient backends over the math fallback.

    Behavior changes vs. the original implementation:
    - When `ctx_mask is None`: dispatch with `is_causal=False` and an empty backend
      preference list, so PyTorch picks Flash / mem-efficient automatically.
    - When `ctx_mask is not None`:
        * Convert bool masks to additive float masks (`0.0` / `-inf`). This is a
          required step for the memory-efficient backend on ROCm; bool masks are
          quietly rejected and fall back to math.
        * Wrap the SDPA call in `sdpa_kernel([FLASH, EFFICIENT, MATH])` so PyTorch
          tries the fast kernels first and only falls back to math if neither
          Flash nor mem-efficient supports the current mask/shape.
    """
    ...
    if ctx_mask is None:
        attn_mask = None
    elif ctx_mask.dtype == torch.bool:
        # bool=True ⇒ visible. Convert to additive float bias so the
        # memory-efficient backend can accept it (it rejects bool masks).
        attn_mask = torch.zeros_like(ctx_mask, dtype=q.dtype)
        attn_mask = attn_mask.masked_fill(~ctx_mask, float("-inf"))
    else:
        attn_mask = ctx_mask.to(dtype=q.dtype)

    # One-shot probe: log the actual SDPA kernel selected on the first call (rank0 only).
    if not _SDPA_PROBE_DONE:
        _probe_sdpa_backend(q, k, v, attn_mask)
