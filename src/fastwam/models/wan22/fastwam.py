...
def _resolve_joint_steps(
    num_inference_steps: int,
    num_video_inference_steps: Optional[int],
    num_action_inference_steps: Optional[int],
    method: str,
) -> int:
    """Resolve a single step count for joint video+action denoise loops.

    The joint denoise loop zips video and action timesteps together, so it cannot
    accept different step counts. If the caller passes both override params and
    they disagree, raise; otherwise fall back to `num_inference_steps`.
    """
    v = num_video_inference_steps
    a = num_action_inference_steps
    if v is not None and a is not None and int(v) != int(a):
        raise ValueError(
            f"`{method}` runs a joint video+action denoise loop and cannot use different "
            f"step counts (got num_video_inference_steps={v}, num_action_inference_steps={a}). "
            "Pass equal values, only one of them, or rely on `num_inference_steps`."
        )
    return int(v if v is not None else (a if a is not None else num_inference_steps))
...
class FastWAM(torch.nn.Module):
    ...
    def infer_action(
        ...
        self.eval()
        del num_video_inference_steps  # action-only path; no video stage
        action_steps = int(
    ...
    def load_checkpoint(self, path, optimizer=None):
        ...
        logger.info("[timing] torch.load start: %s", path)
        payload = torch.load(path, map_location="cpu")
        logger.info("[timing] torch.load done in %.2fs (keys=%s)", time.perf_counter() - _t0, list(payload.keys()))

        _t0 = time.perf_counter()
        if "mot" in payload:
            self.mot.load_state_dict(payload["mot"], strict=False)
            logger.info("[timing] mot.load_state_dict done in %.2fs", time.perf_counter() - _t0)
        elif "dit" in payload:
            logger.warning("Loading legacy `dit` checkpoint into video expert only.")
            self.video_expert.load_state_dict(payload["dit"], strict=False)
            logger.info("[timing] video_expert.load_state_dict (legacy) done in %.2fs", time.perf_counter() - _t0)
        else:
        ...
        if self.proprio_encoder is not None:
            if "proprio_encoder" in payload:
                ...
                logger.info("[timing] proprio_encoder.load_state_dict done in %.2fs", time.perf_counter() - _t0)
            else:
        ...
        if optimizer is not None and "optimizer" in payload:
            ...
            logger.info("[timing] optimizer.load_state_dict done in %.2fs", time.perf_counter() - _t0)
        return payload
