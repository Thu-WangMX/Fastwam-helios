...
def hash_state_dict(state_dict, with_shape=True):
    """Compute model hash directly from an already-loaded state_dict (zero I/O)."""
    keys_dict = _convert_state_dict_to_keys_dict(state_dict)
    keys_str = _convert_keys_dict_to_single_str(keys_dict, with_shape=with_shape).encode("UTF-8")
    return hashlib.md5(keys_str).hexdigest()
